...
void RobotDetection::get2MajorPointsDistance(Mat depthMatrix, vector<Point2f> validRobotPosition, Vec2f &trueLeftPosition, Vec2f &trueRightPosition) {
...
vector<Point2f> RobotDetection::findChessboard(Mat img) {
    ...
    bool found = findChessboardCorners(img, size, pointBuf, CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_NORMALIZE_IMAGE);
    if (!found) {
