...
void initUART(void)
{
    ...
    GPIOPinConfigure(GPIO_PA0_U0RX); //Pin 0 du port A utilis√© pour le RX du UART 
    GPIOPinConfigure(GPIO_PA1_U0TX); //Pin 1 du port A utilis√© pour le TX du UART
...
void uartEcrireOctet(char octet)
{
	...
	{
		//attend que la transmit FIFO se libËre
	}
