...
double Matrix::RowReduceToEchelonForm(){
   ...
   for(int Row = 0; Row < MaxNbOfPivots; Row++){
      ...
      for(int Col=LastPivCol+1; Col<NbCol; Col++){
         ...
         //If the max element is not zero we have a nonzero column
         //Also, select this max element as the pivot
         if( ((RowMaxEl>0.0)?RowMaxEl:-RowMaxEl) > MATRIX_ZERO_LIM ){

            //Place the selected element at the pivot position
            if(RowMax != Row){
      ...
      //If a pivot column is found, create zeros in all pos. below the pivot
      if(PivCol != -1){
   ...
   //Return the detreminant change factor caused by row operations
...
double Matrix::RowReduceToReducedEchelonForm(){
   ...
   for(int Row = (NbRow-1); Row >= 0; Row--){
      ...
      //Find the leading entry in that Row
...
int Matrix::operator==(const Matrix &_m) const{
   ...
   int Equal = 1; //We suppose the matrix are equal
...
Matrix Matrix::operator+(const Matrix &_m) const{
   ...
   //If the two matrix are not of the same size, an empty matrix is returned
...
const Matrix &Matrix::operator+=(const Matrix &_m){
   ...
   //If the two matrix are not of the same size, the current matrix is set
   //empty to indicate an error
   if( (NbRow == _m.NbRow)&&(NbCol == _m.NbCol) ){
...
Matrix Matrix::operator*(const Matrix &_m2) const{
   ...
   //If there is a mismatch between the two matrix, an empty matrix is returned
   //to indicate an error
...
const Matrix &Matrix::operator*=(const Matrix &_m2){
   ...
   //If there is a mismatch between the two matrix, the current matrix is set
   //empty to indicate an error
   if(NbCol != _m2.NbRow){
